...
class Helper
{
    ...
    static function basliyorMu($haystack, $needle) {

        return substr_compare($haystack, $needle, 0, strlen($needle)) === 0;

    }

    static function bitiyorMu($haystack, $needle) {

        return substr_compare($haystack, $needle, -strlen($needle)) === 0;

    }

    static function iceriyorMu($text, $char){

        return stripos($text, $char) !== FALSE;

    }

    static function buyuk_yap($text){
    ...
    static function excelToPhpDate($text) {
        $text = str_ireplace(' ', '', $text);

        if(is_numeric($text)) {
            $UNIX_DATE = ($text - 25569) * 86400;
            return gmdate("Y-m-d", $UNIX_DATE);
        }
        else if(Helper::iceriyorMu($text, '.')) {
            $dates = explode('.', $text);
            $date = str_pad($dates[0], 2, '0', STR_PAD_LEFT).'-'.str_pad($dates[1], 2, '0', STR_PAD_LEFT).'-'.str_pad($dates[2], 4, '0', STR_PAD_LEFT);
    ...
    static function tarihFarki($tarih1, $interval)
    {
        $start_date = Carbon\Carbon::now();
        $end_date = Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $tarih1);

        if($interval = 'dk') {
            $different = $start_date->diffInMinutes($end_date);
        }
        elseif($interval = 'sa') {
            $different = $start_date->diffInHours($end_date);
        }
        elseif($interval = 'gn') {
            $different = $start_date->diffInDays($end_date);
        }
        else {
            $different = $start_date->diffInMinutes($end_date);
        }
