                ...
                <ul class="nav" ui-nav>
                  ...
                  <span class="nav-caret">
                    ...
                    @if(Helper::GeneralWebmasterSettings("newsletter_status"))
                        @if(@Auth::user()->permissionsGroup->newsletter_status)
                            <?php
                            $currentFolder = "8/topics"; // Put folder name here
                            $PathCurrentFolder = substr($urlAfterRoot, 0, strlen($currentFolder));
                            ?>
                            <li {{ ($PathCurrentFolder==$currentFolder) ? 'class=active' : '' }}>
                                <a href="{{ url('/admin/8/topics') }}">
                                    <span class="nav-icon">
                                        <i class="material-icons">&#xe24b;</i>
                                    </span>
                                    <span class="nav-text">{{ __('backend.products') }}</span>
                                </a>
                            </li>
                        @endif
                    @endif
                  ...
                  <span class="nav-icon">
                    ...
                    @foreach($GeneralWebmasterSections as $GeneralWebmasterSection)
                        @if(in_array($GeneralWebmasterSection->id,$data_sections_arr))
                            ...
                            @else
                                @if($GeneralWebmasterSection->id == 12 || $GeneralWebmasterSection->id == 13 || $GeneralWebmasterSection->id == 14)
									<?php
										if($GeneralWebmasterSection->id == 12) { /*Sezonlar*/
											$currentFolder = "sezons";
										}
										if($GeneralWebmasterSection->id == 13) { /*Ligler*/
											$currentFolder = "ligs";
										}
										if($GeneralWebmasterSection->id == 14) { /*Takımlar*/
											$currentFolder = "takims";
										}
										$PathCurrentFolder = substr($urlAfterRoot, 0, strlen($currentFolder));
									?>
									<li {{ ($PathCurrentFolder==$currentFolder) ? 'class=active' : '' }}>
										@if($GeneralWebmasterSection->id == 12)
											<a href="{{ route('sezons') }}">
										@elseif($GeneralWebmasterSection->id == 13)
											<a href="{{ route('ligs') }}">
										@elseif($GeneralWebmasterSection->id == 14)
											<a href="{{ route('takims') }}">
										@endif
											  <span class="nav-icon">
