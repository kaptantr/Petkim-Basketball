...
<?php
    ...
    if($WebmasterSectionTitle == 'Oyuncu İstatistikleri') {
        $webmasterId = 8;
    }
    else {
        $webmasterId = 1;
        ...
        <div class="box">
                ...
                <ul class="nav">
                    @if(@Auth::user()->permissionsGroup->add_status)
                        <li class="nav-item inline">
                            <a class="btn btn-fw primary" href="{{route("topicsCreate", $webmasterId==0 ? 1 : $webmasterId)}}">
                                <i class="material-icons">&#xe02e;</i>
                                &nbsp; {{ __('backend.topicNew') }} Ekle
                            </a>
                        </li>
                    @endif
                    <li class="nav-item inline">
            ...
            <div>
                ...
                <div class="dker b-b displayNone" id="filter_div">
                    <div class="p-a">
                        {{Form::open(['route'=>['topics', $webmasterId],'method'=>'GET','id'=>'filter_form','target'=>''])}}
                                ...
                                @foreach($WebmasterSection->customFields as $customField)
                                    @if($customField->in_search)
                                        ...
                                        @if($cf_land_active)
                                            ...
                                            @elseif($customField->type == $webmasterId)
                                                ...
                                                <div class="col-md-3 col-xs-6 m-b-5p">
                                                    <select name="{{'customField_'.$customField->id}}"
                                                        ...
                                                        @foreach ($cf_details_lines as $cf_details_line)
                                                            <option value="{{ $line_num  }}" {{ (@$_GET['customField_'.$customField->id] == $line_num) ? "selected='selected'":""  }}>{{ $cf_details_line }}</option>
                                                            <?php
                                                            $line_num++;
                                                            ?>
                                                        @endforeach
                ...
                {{Form::open(['route'=>['topicsUpdateAll', $webmasterId],'method'=>'post'])}}
                <div class="table-responsive">
                    <table class="table table-bordered" style="width: 100%" id="topics_8">
                        <thead class="dker">
                        ...
                        @if($WebmasterSectionTitle == 'Oyuncu İstatistikleri')
                            ...
                            <th>Süre</th>
                            <th>Sayı</th>
                            <th title="Atış Girişimi">AG</th>
                            <th title="Serbest Atış">SA</th>
                            <th title="2 Sayı">S2</th>
                            <th title="3 Sayı">S3</th>
                            <th title="Savunma Ribaundu">SR</th>
                            <th title="Hücum Ribaundu">HR</th>
                            <th title="Toplam Ribaund">TR</th>
                            <th title="Asist">AS</th>
                            <th title="Top Çalma">TÇ</th>
                            <th title="Top Kaybı">TK</th>
                            <th title="Blok">BL</th>
                            <th title="Faul">FA</th>
                            <th title="Verimlilik Puanı">VP</th>
                            <th>Sezon</th>
                            {{--<th>Lig</th>--}}
                            {{--<th>Kapsam</th>--}}
                            <th>Takımı</th>
                        ...
                        @if($WebmasterSectionTitle == 'Oyuncu İstatistikleri')
                            <th class="text-center" style="max-width:150px;">İşlem</th>
                        @else
            ...
            @if($customField->in_statics && ($customField->type==6 || $customField->type==7))
                <div class="box">
                    ...
                    <div id="canvas-holder-{{ $customField->id }}"
                        ...
                        <canvas id="chart-area-{{ $customField->id }}" width="920" height="{{ $heigth }}" style="margin: 0 auto"></canvas>
                    </div>
                </div>
            @endif
        @endforeach
        ...
        $(document).ready(function () {
            ...
            var dataTable = $(table_name).DataTable({
                ...
                @if($WebmasterSectionTitle == 'Oyuncu İstatistikleri')
                    ...
                    { "data": "adsoyad", "orderable": true },
                    { "data": "rakip_takim", "orderable": true },
                    { "data": "tarih", "orderable": true },
                    { "data": "sure", "orderable": true },
                    { "data": "sayi", "orderable": true },
                    { "data": "AG", "orderable": true },
                    { "data": "SA", "orderable": true },
                    { "data": "S2", "orderable": true },
                    { "data": "S3", "orderable": true },
                    { "data": "SR", "orderable": true },
                    { "data": "HR", "orderable": true },
                    { "data": "TR", "orderable": true },
                    { "data": "AS", "orderable": true },
                    { "data": "TÇ", "orderable": true },
                    { "data": "TK", "orderable": true },
                    { "data": "BL", "orderable": true },
                    { "data": "FA", "orderable": true },
                    { "data": "VP", "orderable": true },
                    { "data": "sezon", "orderable": true },
                    // { "data": "lig", "orderable": true },
                    // { "data": "kapsam", "orderable": true },
                    { "data": "takimi", "orderable": true },
            ...
            @if($WebmasterSectionTitle == 'Oyuncu İstatistikleri')
                const scrollBooster = new ScrollBooster({
                    viewport: document.querySelector('.table-responsive'),
                    content: document.querySelector('table#topics_8'),
                    scrollMode: 'native',
